...
function feature.install(mod, services)
  ...
  local function drawExpBar(battle, state, context)
    ...
    local rendererHud = context.rendererHud
    if rendererHud then
      if px <= 0 then return end
      local anchors = rendererHud.anchors
      local scale = anchors.playerScale or rendererHud.playerScale
        or rendererHud.scale
      local x = anchors.expRight - px * scale
      local width = px * scale
      local textRect = rendererHud.text and
        (battle.phase == "moveSelect" and rendererHud.text.moves
          or battle.phase == "mimicSelect" and rendererHud.text.mimic)
      if textRect then
        local coverEnd = textRect[1] + textRect[3]
        if x < coverEnd then
          width = width - (coverEnd - x)
          x = coverEnd
        end
        ...
        x, width = clipForMenu(
          battle.phase, x, width, anchors.textLeft, anchors.textScale)
      end
      if width <= 0 then return end
      love.graphics.setCanvas(rendererHud.canvas)
      love.graphics.setShader()
      love.graphics.setColor(color[1], color[2], color[3], color[4])
      love.graphics.rectangle(
        "fill", x, anchors.expY, width, 2 * scale)
      return
    end
    -- Compatibility fallback for older overlay-service consumers.  Current
    -- KASC renderer paths always provide the validated rendererHud profile.
