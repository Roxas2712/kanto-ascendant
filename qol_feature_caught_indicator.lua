...
function feature.install(mod, services)
  ...
  local derivedBall = "save/mod-derived/" .. mod.id .. "/ui/ball.png"
  local ballImage, ballQuad

  local function ballAsset()
    if ballImage == false then return nil end
    if not ballImage then
      local ok, image = pcall(love.graphics.newImage, derivedBall)
      if not ok then
        ballImage = false
  ...
  local function drawCaughtIndicator(battle, state, context)
    ...
    if rendererHud then
      ...
      scale = anchors.enemyScale or rendererHud.enemyScale
        or rendererHud.scale
      -- The explicit profile has already converted HUD_RECT.enemy into the
      -- snapped panel coordinate; retain only the live name width and shake.
      x = anchors.caughtNameOrigin + (enemyNameX(battle) + hudShake) * scale
      y = anchors.caughtY
      love.graphics.setCanvas(rendererHud.canvas)
    elseif voxel3dBattleData then
      scale = voxel3dBattleData.scale
      x = (enemyNameX(battle) - 9 + hudShake) * scale
    ...
    if not rendererHud and not voxel3dBattleData then
