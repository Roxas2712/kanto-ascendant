...
return function(mod,opts)
 ...
 function M.build(id,selection)
  ...
  if W.plain(selection)then return original,{frameWidth=16,frameHeight=16,frames=6,anchorX=8,anchorY=16}end
 ...
 function M.bind(game,player,id,original)
  if W.presentation.mode(W.get(id))~='2d'then return false end
  if not W.isActive()or W.plain(W.get(id))then return false end
