...
return function(mod, opts)
  ...
  function C.setSecondaryChanceResolver(resolve)
    C.secondaryChance=assert(resolve)
  end
  ...
  local function registerForesight()
    ...
    mod.content.move_effects:register(id, {
      ...
      run=function(ctx)
        ...
        if target[identifiedKey] or identification and identification.marked(ctx.battle,target)
            or target.invulnerable or target.mon.hp<=0 then
          return {tr("But, it failed!","Doch es schlug fehl!")}
        end
