return function(mod,opts)
  ...
  function M.preview(game,id,row,selection)
    ...
    function screen:update(dt)
      ...
      if W.appearanceReady and input:wasPressed('select')then
    ...
    function screen:draw()
      ...
      Font.draw(W.appearanceReady and tr('SEL:PARTS LR:TURN','SEL:TEILE LR:DREH')or tr('L/R:TURN','L/R:DREHEN'),4,132)
      G.setColor(1,1,1,1)
    end
  ...
  function M.new(game)
    ...
    local function items()
      ...
      if W.appearanceReady then
        result[#result+1]={label=tr('CHOOSE INDIVIDUAL PARTS','EINZELTEILE WAEHLEN'),parts=true}
      end
      result[#result+1]={label=tr('SPRITE STYLE','DARSTELLUNG'),style=true,
        right=current.style=='native'and'VANILLA'or'AUTO'}
      for _,row in ipairs(W.rows(id))do
        result[#result+1]={label=tr(row.en,row.de),row=row,
          right=current.outfit==row.id and'*'or nil}
      end
    ...
    menu=List.new(game,tr('WARDROBE','KLEIDERSCHRANK'),items(),{
      ...
      onChoose=function(item)
        ...
        if item.style then
          selection.style=selection.style=='native'and'hd'or'native'
          M.preview(game,id,W.catalog.find(id,selection.outfit)or{id='original',en='Original',de='Original'},selection)
          return
        end
        if item.parts then
          local preview=M.preview(game,id,W.catalog.find(id,selection.outfit)or{id='original',en='Original',de='Original'},selection)
          M.customize(game,id,preview);return
        end
        selection.outfit=item.row.id
        ...
        if not W.appearanceReady then selection={outfit='original',head='classic',style=selection.style,hair='natural',streak='none',eyewear='none',hairstyle='standard',upper='preset',lower='preset',footwear='preset'}end
