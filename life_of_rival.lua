...
return function(mod, opts)
  ...
  local function normalizeVisit(value, mapId)
    ...
    local repeats = {}
    for actor in pairs(seen) do
      repeats[actor] = clamp(type(out.repeatTalks) == "table"
  ...
  local function normalize(raw)
    ...
    local hadIndividualIntroductions = type(s.thirdIntroductions) == "table"
    ...
    if s.thirdIntroduced and not hadIndividualIntroductions then
    ...
    s.visitMap = type(s.visitMap) == "string" and s.visitMap ~= ""
      and s.visitMap or nil
    local heard = {}
  ...
  local function encounterSpecies(game, mapId)
    ...
    local found = {}
    -- Read only the native, active land/surf slot tables. Recursing through
    -- arbitrary metadata also advertised disabled/time-of-day/event pools.
    -- Do not roll encounters here: that would consume RNG or event receipts.
    for _, kind in ipairs({ "grass", "water" }) do
      local pool = root[kind]
      if type(pool) == "table" and tonumber(pool.rate)
          and tonumber(pool.rate) > 0 and type(pool.slots) == "table" then
        local buckets = pool.buckets or game.data.field
          and game.data.field.constants and game.data.field.constants.encounterBuckets
          or { 51, 102, 141, 166, 191, 216, 229, 242, 253, 256 }
        local previous = 0
        for index, slot in ipairs(pool.slots) do
          local threshold = tonumber(buckets[index]) or previous
          local reachable = math.min(threshold, 256) > previous
          previous = math.max(previous, threshold)
          local id = type(slot) == "table" and slot.species
          if reachable and type(id) == "string"
              and game.data.pokemon and game.data.pokemon[id] then
            found[id] = true
          end
        end
      end
  ...
  function L.routeSearchClaim(game, actor, mapId, context)
    ...
    if not (CHARACTER_IDS[actor] and label and #species > 0)
        or mapGroup(mapId) == "city" then return nil end
    local templates = {}
  ...
  -- Both live controllers share follow-up copy. Deterministic rotation is
  -- stored by the visit owner; talking never rerolls a rare hint or challenge.
  function L.followupDialogue(game, actor, mapId, count, hint)
    if not CHARACTER_IDS[actor] then return nil end
    count = math.max(1, integer(count, 1))
    if count==1 and opts.hints then
      local line=opts.hints.dialogue(game,actor,mapId,hint)
      if line then return line end
    end
    local city = mapGroup(mapId) == "city"
    local species = not city and encounterSpecies(game, mapId) or {}
    if not city and count % 2 == 0 then
      if #species > 0 then
        local search = L.routeSearchClaim(game, actor, mapId, {
          speciesPick = (math.floor(count / 2) - 1) % #species + 1,
          templatePick = (math.floor(count / 2) - 1) % 3 + 1,
        })
        if search then return search end
      end
    end
    local suffix = city and "followup_city" or "followup_route"
    local cycle = #species == 0 and count or math.ceil(count / 2)
    if cycle % 2 == 0 then suffix = suffix .. "_2" end
    return localized(assert(DATA_BY_ID[actor:lower() .. "_" .. suffix]))
  end
  ...
  function L.syncKnownActors(s, game)
    ...
    if ngplusActive(game and game.save) then
      -- New Game can normalize the provisional avatar before the archived
      -- protagonist is selected. A former self is not an introduction receipt.
      for actor in pairs(CHARACTER_IDS) do
        s.knownActors[actor] = actor == roles.player
          or L.hasThirdIntroduction(s, actor)
      end
    end
  ...
  function L.commitDialogue(s, actor, role, row)
    ...
    if row.kind == "intro" then
      ...
      if role == "third" then s.thirdIntroduced = true end
      s.knownActors = type(s.knownActors) == "table" and s.knownActors or {}
  ...
  -- A receipt belongs to a stay, not permanently to a route. Preserve the
  -- current map across reloads, but retire visits even when leaving for an
  -- interior/ineligible map. Old saves have no visitMap: keep only the receipt
  -- for the map actually entered, rather than reviving a historical tour.
  function L.enterVisitMap(s, mapId)
    if not (s and type(mapId) == "string" and mapId ~= "") then return false end
    if s.visitMap == mapId then return false end
    local current = not s.visitMap and s.visitReceipts
      and s.visitReceipts[mapId] or nil
    s.visitReceipts = current and { [mapId] = current } or {}
    s.visitMap = mapId
    persist(s)
    return true
  end
    ...
    if mode ~= "none" then
      ...
      for _, actor in ipairs(actorList) do
        ...
        visit.actors[#visit.actors + 1] = {
          ...
          duelPlan = visit.duel and copy(battlePlan) or nil,
          battleOffered = offered or (visit.duel and visit.nextChallenge
            and battlePlan ~= nil), battleOfferRoll = offerRoll,
        }
      end
    end
    visit.rareHint=not visit.duel and opts.hints
      and opts.hints.prepare(game,mapId,visit.actors,visit.token,randomInt) or false
    s.visitReceipts[mapId] = visit
    persist(s)
    return visit, true
  end

  local function retainFinishedAppearance(s, pending)
    local visit = {
      token = pending.token, mapId = pending.mapId, mode = "single",
      actors = {{ actor = pending.actor, role = pending.role,
        dialogue = copy(pending.dialogue), battleOffered = false }},
      talked = { [pending.actor] = true },
      battleResolved = { [pending.actor] = true }, battleStarted = {},
      spawnCounted = pending.spawnCounted == true, duel = false,
      nextChallenge = false,
    }
    local intro = pending.arc == "intro"
    L.scheduleNext(s, pending.mapId)
    if intro then s.introArmedMap = nil end
    s.visitMap = pending.mapId
    s.visitReceipts = { [pending.mapId] = visit }
    return visit
  end
  ...
  local function sourceParty(game, actor)
    ...
    local roles=L.characterRoles(game)
    if edition()~='yellow' and roles and actor==roles.third then
      -- The story rival owns Oak's counter-starter; the third protagonist
      -- owns the remaining ball, not another copy of that same counter.
      -- Red's authored final Mt. Silver party still keeps all three starters.
      branch=branch%3+1
    end
  ...
  function L.spawnVisit(game, s, visit)
    ...
    local entries, cells = {}, {}
    local duelCells, duelLayout
    if visit.duel and duelScene then
      duelLayout=duelScene.reserve(function(rejected)
        return L.findSpawnCell(game,ow,rejected,1)
      end,function(x,y)return safeCell(game,ow,x,y)end)
      if not duelLayout then return false,'NO_DUEL_LAYOUT' end
      duelCells=duelLayout.trainers
    elseif visit.duel then
    ...
    if duelLayout then
      -- Older visit receipts did not retain ordinary duel teams. Materialize
      -- once on migration; new receipts already carry their exact plans.
      for _,row in ipairs(visit.actors)do
        if not row.duelPlan then row.duelPlan=copy(row.battlePlan
          or L.buildBattlePlan(game,row.actor,s))end
      end
      local started,reason=duelScene.start(active,duelLayout,function()
        if active.game.save~=active.save then return false end
        local current=L.state(false)
        local receipt=current and current.visitReceipts[active.mapId]
        if not receipt or receipt.token~=active.token then return false end
        -- Conversation normalization creates fresh receipt tables. Merge only
        -- presentation fields, never overwrite newer talks/optional outcomes.
        receipt.duelPresentation=copy(active.visit.duelPresentation)
        receipt.duelProgress=copy(active.visit.duelProgress)
        return persist(current)
      end)
      if not started then
        if mod.log and mod.log.warn then mod.log:warn('Rival duel: '..tostring(reason))end
        return rollbackSpawn()
      end
    end
  ...
  function L.cleanup(active)
    ...
    if duelScene then duelScene.cleanup(active)end
  ...
  function L.trySpawn(game)
    ...
    if s.pending and s.pending.talked and not s.pending.battleStarted then
      ...
      if L.VISIT_CONTRACT_ENABLED then
        local retained = retainFinishedAppearance(s, s.pending)
        if not persist(s) then return false end
        return L.spawnVisit(game, s, retained)
      end
  ...
  local function finishAppearance(active, result, done)
    ...
    if active.finished then return false end
    local s = L.state(true)
    ...
    if L.VISIT_CONTRACT_ENABLED then
      local visit = retainFinishedAppearance(s, s.pending)
      persist(s)
      active.finished, active.visit, active.talking = true, visit, false
      active.entries = {{ npcId = active.npcId, npc = active.npc,
        actor = active.actor, row = visit.actors[1] }}
      releaseIntroLock(active)
  ...
  local function releaseVisitTalk(active, entry, done)
    ...
    if duelScene then duelScene.releaseTalk(active,entry and entry.actor)end
  ...
  local function startBattle(active, pending, done)
    ...
    if L.active ~= active or active.finished or active.save ~= game.save
        or not (active.ow and active.ow.map and active.ow.map.id == active.mapId)
        or pending.battleStarted then return false end
  ...
  startVisitBattle = function(active, entry, visit, done)
    ...
    if L.active ~= active or active.save ~= game.save
        or not (active.ow and active.ow.map and active.ow.map.id == active.mapId)
        or visit.battleResolved[actor] == true then return false end
    ...
    if not (current and current.token == visit.token
        and current.battleStarted[actor] ~= true
        and current.battleResolved[actor] ~= true) then
      releaseVisitTalk(active, entry, done)
      return false
    end
    ...
    local function closeFailedAttempt()
      -- Battle decoration can normalize the save bucket. Do not write to
      -- a detached pre-decoration visit or discard newer peer receipts.
      local state = L.state(true)
      local receipt = state and state.visitReceipts
        and state.visitReceipts[active.mapId]
      if receipt and receipt.token == active.token then
        receipt.battleResolved[actor] = true
        persist(state)
      end
      releaseVisitTalk(active, entry, done)
      return false
  ...
  function L.handleVisitTalk(game, ow, npc, done)
    ...
    local entry = active and active.visit and (visitEntryFor(active, npc)
      or duelScene and duelScene.entryFor(active,npc))
    ...
    local duelText=duelScene and duelScene.talk(active,actor)
    if duelText then
      -- Ending an in-flight presentation action commits its checkpoint and
      -- normalizes receipt tables. Acquire that current receipt before
      -- recording this conversation, not the detached pre-action table.
      s=L.state(true)
      visit=s and s.visitReceipts and s.visitReceipts[active.mapId]
      if not visit or visit.token~=active.token then
        releaseVisitTalk(active,entry,done);return false
      end
    end
    ...
    local function recordShown(shown)
      if not shown then
        releaseVisitTalk(active, entry, done)
        return false
      end
    ...
    if not canChallenge then
      return recordShown(pushText(game, duelText or row.dialogue.text, function()
        releaseVisitTalk(active, entry, done)
      end))
    end
    local promptRow = DATA_BY_ID[actor:lower() .. "_battle_prompt"]
    local prompt = promptRow and tr(promptRow.en, promptRow.de) or ""
    local choiceConsumed = false
    return recordShown(pushText(game, (duelText or row.dialogue.text) .. "\f" .. prompt, nil, {
      defaultNo = true,
      choice = function(yes)
        if choiceConsumed or L.active ~= active or active.save ~= game.save
            or not (ow.map and ow.map.id == active.mapId) then return false end
        local currentState = L.state(true)
        local currentVisit = currentState and currentState.visitReceipts
          and currentState.visitReceipts[active.mapId]
        if not currentVisit or currentVisit.token ~= active.token
            or currentVisit.battleResolved[actor] == true
            or currentVisit.battleStarted[actor] == true then
          releaseVisitTalk(active, entry, done)
          return false
        end
        choiceConsumed = true
        s, visit = currentState, currentVisit
        ...
        if text then
          local shown = pushText(game, text, function()
            releaseVisitTalk(active, entry, done)
          end)
          if not shown then releaseVisitTalk(active, entry, done) end
  ...
  function L.onMapEntered(game, mapId)
    ...
    if L.active and (L.active.mapId ~= mapId
        or L.active.save ~= (game and game.save)
        or L.active.ow ~= (game and game.overworld)) then L.cleanup(L.active) end
    if not L.eligible(game) then return false end
    local s = L.state(true)
    if not s then return false end
    if L.VISIT_CONTRACT_ENABLED then L.enterVisitMap(s, mapId) end
    abandonDifferentMap(s, mapId)
    -- Do not replace a just-spoken introduction with a new random encounter
    -- on save reload before its completion callback has run.
    if L.VISIT_CONTRACT_ENABLED and s.pending and s.pending.talked
        and not s.pending.battleStarted then
      retainFinishedAppearance(s, s.pending)
      persist(s)
    end
  ...
  if mod.hooks and type(mod.hooks.wrap) == "function" then
    ...
    L.introWatchdogUnwrap = mod.hooks:wrap("input.step",
      function(nextStep, game, dt)
        ...
        if L.active and duelScene then duelScene.tick(L.active,dt)end
        return result
      end, 4550)
  end
