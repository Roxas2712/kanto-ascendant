...
function M.new(d)
  ...
  function self:activate(raw,p,mount)
    ...
    for _,f in ipairs(m.files) do
      ...
      if d.sha256(table.concat(parts))~=f.sha256 then return false,"file_hash_mismatch" end
  ...
  function self:materialize(owner,path)
    ...
    if existing and d.sha256(existing)==f.sha256 then return target end
    ...
    if d.sha256(raw)~=f.sha256 or not write(target,raw) then return nil,"materialize_failed" end
