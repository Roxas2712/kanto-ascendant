...
return function(mod)
  ...
  local function modernOverlay(game, page)
    ...
    -- A light translucent pass retains the engine-owned text and sprite
    -- geometry while giving both original pages the same FireRed-family
    -- panel hierarchy as the Bag and Storage skins.
    color(C.blue, .10)
    love.graphics.rectangle("fill", 0, 0, 160, 17)
    color(C.aqua, .10)
    love.graphics.rectangle("fill", 0, 64, 78, 80)
    color(C.cream, .11)
    love.graphics.rectangle("fill", 79, 64, 81, 80)
  ...
  if okParty and PartyMenu and not PartyMenu.__ascendantModernSkin then
    ...
    PartyMenu.draw = function(self)
      ...
      color(C.blue, .08)
      love.graphics.rectangle("fill", 0, 0, 23, 96)
      for index = 1, math.min(6, #(self.party or self.game.save.party)) do
        if index % 2 == 0 then
          color(C.aqua, .07)
          love.graphics.rectangle("fill", 23, (index - 1) * 16, 137, 16)
        end
      end
      color(C.coral, .20)
      love.graphics.rectangle("line", .5, .5, 159, 143)
