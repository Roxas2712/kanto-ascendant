...
VoxelAdapter.POKEMON_DEFERRED = "VOXEL_BODY_DEFERRED"
...
-- A newly created visible Wild exists logically before its short spawn FX is
-- allowed to expose the body.  `Entity:getWorldSprite()` deliberately returns
-- nil during this window and `Entity:pose()` does the same.  That is a normal
-- lifecycle state, not a missing asset.  Keep this predicate narrow so a
-- settled entity with a genuinely missing sprite still takes the loud
-- emergency path below.
function VoxelAdapter.isTransientBodyHidden(entity)
  if not entity or entity.hiddenEncounter or entity.visibleSprite == false then
    return false
  end
  if entity.hiddenBody == true then return true end
  local fx = entity.spawnFx
  return type(fx) == "table" and fx.done ~= true and fx.bodyShown ~= true
end

local function worldSpriteOf(entity)
  if type(entity and entity.getWorldSprite) == "function" then
    local ok, sprite = pcall(entity.getWorldSprite, entity)
    if not ok then
      return nil, "getWorldSprite() threw: " .. tostring(sprite)
    end
    -- Do not use `or entity.sprite` here. nil is the intentional contract
    -- while spawn FX hides the body; falling through would make isPoseSafe
    -- accept a body that pose() must refuse on the next line of the pipeline.
    return sprite
  end
  return entity and entity.sprite or nil
end
...
function VoxelAdapter.isPoseSafe(entity)
  ...
  local sprite, spriteErr = worldSpriteOf(entity)
  if sprite == nil then
    if VoxelAdapter.isTransientBodyHidden(entity) then
      return false, "body hidden by active spawn FX", "deferred"
    end
    return false, spriteErr or "sprite is nil", "error"
  end
...
function VoxelAdapter.probePose(entity)
  local okSafe, why, disposition = VoxelAdapter.isPoseSafe(entity)
  if not okSafe then return false, why, disposition end
...
function VoxelAdapter:deferEntity(entity, reason)
  if not entity then return false end
  entity.voxelDeferred = true
  entity.voxelDeferredReason = tostring(reason or "spawn FX body hidden")
  entity.voxelLastError = nil
  entity.pokemonRenderer = VoxelAdapter.POKEMON_DEFERRED
  entity.worldRenderer = VoxelAdapter.WORLD_RENDERER
  entity.depthIntegration = "PENDING"
  entity.objectOcclusion = "PENDING"
  ...
  entity.worldSpriteAdapterStatus = "DEFERRED"
  entity.grassRenderer = "PENDING"
  local d = RenderDiagnostics.ensure(entity)
  d.dramaticBillboardDeferred = (d.dramaticBillboardDeferred or 0) + 1
...
function VoxelAdapter:markFallback(entity, err)
  ...
  entity.voxelDeferred = nil
  entity.voxelDeferredReason = nil
...
function VoxelAdapter:updateEntity(entity)
  ...
  local okPose, detail, disposition = VoxelAdapter.probePose(entity)
  if not okPose then
    if disposition == "deferred" then
      return self:deferEntity(entity, detail)
    end
  ...
  entity.voxelLastError = nil
  entity.render2DFallback = false
  entity.worldSpriteAdapterStatus = entity.nativeSpriteRenderer
    and "NATIVE" or "LEGACY"
  entity.worldBillboardReady = isWorldBillboard(entity.pokemonRenderer)
  entity.voxelSource = entity.spriteSource2D
  return true
end
...
local function filterOverlayEmergencyEntities(entities)
  ...
  for _, e in ipairs(entities or {}) do
    ...
    local wild = VoxelAdapter.isWildEntity(e)
    local isEmerg = wild and isEmergencyOverlay(e.pokemonRenderer)
    local isDeferred = wild and e.voxelDeferred == true
    if isEmerg or isDeferred then
      if isDeferred then
        d.dramaticBillboardDeferred = (d.dramaticBillboardDeferred or 0) + 1
        d.lastFailureReason = nil
      else
...
function VoxelAdapter.statusLines(entity)
  if not entity then return {} end
  local lines = {}
  local sprite = worldSpriteOf(entity)
