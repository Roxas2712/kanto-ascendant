...
-- Find the first step of a bounded shortest route to a collision-safe cell
-- adjacent to the player. The former greedy-only planner could wait forever
-- behind one NPC/follower even when a one-tile detour was open. This fallback
-- uses the exact same canStep/occupancy/warp/water/cave gates as the ordinary
-- step and never reserves while searching.
local function routeStepToward(entity, map, entities, player, tx, ty,
                               region, allowLeave, opts)
  opts = opts or {}
  if not (entity and map and tx ~= nil and ty ~= nil) then return nil end
  local sx, sy = entity.cellX, entity.cellY
  local startDistance = math.abs(sx - tx) + math.abs(sy - ty)
  if startDistance <= 1 then return nil end

  local queue = { { x = sx, y = sy } }
  local head = 1
  local seen = { [tostring(sx) .. ":" .. tostring(sy)] = true }
  local maxNodes = math.max(32, tonumber(opts.pathNodeLimit) or 256)
  local visited = 0

  while head <= #queue and visited < maxNodes do
    local cur = queue[head]
    head = head + 1
    visited = visited + 1
    local dirs = chaseStepCandidates(cur.x, cur.y, tx, ty, {
      expandOrthogonal = true,
      allowEnterWater = opts.allowEnterWater,
    })
    for _, d in ipairs(dirs) do
      ...
      if not seen[key] then
        seen[key] = true
        local ok, reason = canStep(map, entities, entity, player, nx, ny,
                                   region, allowLeave, opts)
        if ok then
          local firstX = cur.firstX or nx
          local firstY = cur.firstY or ny
          local firstReason = cur.firstReason or reason
          if math.abs(nx - tx) + math.abs(ny - ty) <= 1 then
            local bx = entity.behaviorState
            if bx then
              bx.path = {
                fromX = sx, fromY = sy, goalX = nx, goalY = ny,
                firstX = firstX, firstY = firstY, visited = visited,
              }
            end
            return {
              nx = firstX, ny = firstY,
              dx = firstX - sx, dy = firstY - sy,
              reason = firstReason,
            }
          end
          queue[#queue + 1] = {
            x = nx, y = ny, firstX = firstX, firstY = firstY,
...
local function stepToward(entity, map, entities, player, tx, ty, region, allowLeave, chase, opts)
  ...
  -- Direct reducing steps are preferred. Only when all are blocked do the
  -- bounded route search consider a lateral/away first step around an object.
  if #scored == 0 then
    local route = routeStepToward(entity, map, entities, player, tx, ty,
                                  region, allowLeave, opts)
    if route then
      scored[1] = {
        dx = route.dx, dy = route.dy, nx = route.nx, ny = route.ny,
        reason = route.reason,
        enterWater = route.reason == "enter_water",
        score = -1,
      }
    end
  end
...
local function tickLandAggressive(entity, ctx, bx, t)
  ...
  if bx.state == Behavior.STATE.CHASING or bx.chasing then
    ...
    if not moved then
      ...
      bx.chaseStallCount = (bx.chaseStallCount or 0) + 1
      if isTemporaryOccupancyBlock(block) then
        -- Temporary blocker at shore: wait briefly, keep chase alive.
        bx.nextActionAt = now() + 0.1
        -- A follower/NPC can also remain parked forever. Give short-lived
        -- traffic time to clear, then release sight so this spawn can resume
        -- its normal search instead of owning an immortal chase state.
        if bx.chaseStallCount > 90 then
          leaveChase(entity, bx, t)
        end
      else
