...
function SpawnLogic:resolveWaterSprite(speciesId, isShiny, form, opts)
  ...
  local trySubmerged = style == "followers" and function()
    ...
    if not redpp then
      ...
      if type(V.exists) == "function" and V.exists(rel) then
...
function SpawnLogic:shouldSuppressClassicEncounter(ctx)
  ...
  -- "Wild Pokémon" is the master switch for this visible-spawn pipeline.
  -- If it is off, classic encounters are the fallback and must never inherit
  -- the independent default `random_encounters=false`; otherwise both
  -- systems are disabled and authored caves (including HEVO trials) become
  -- completely empty.
  ...
  -- Random Encounters OFF is only safe once this map's visible-spawn
  -- pipeline is demonstrably READY.  A renderer/asset/reachability failure
  -- calls _restoreVanillaEncounters(), but the old hook ignored that state
  -- and continued returning nil from encounter.roll.  That left authored
  -- caves with neither visible Pokémon nor classic battles.  Fail open to
  -- the native encounter table until (and unless) this exact map finishes
  -- initialization; the next successful map entry can suppress it again.
  local st = self.state
  if not st or st.mapId ~= mapId or st.fallbackToVanilla == true
      or st.initialized ~= true or st.pipelineVerified ~= true then
    return false
  end
...
function SpawnLogic:initializeForMap(mapId, game)
  ...
  local maxVisible = Config.maxVisible(self.mod)
  local encounterMax = tonumber(encDef and encDef.kaMaxVisible)
  if encounterMax and encounterMax >= 0 then
    maxVisible = math.min(maxVisible, math.floor(encounterMax))
  end
  self.targetSpawnCount = SpawnRegions.targetCount({
    eligibleTiles = #self.eligibleCache,
    minVisible = math.min(Config.minVisible(self.mod), maxVisible),
...
function SpawnLogic:trySpawn(game, opts)
  ...
  -- A map can impose a stricter presentation cap than the global Wilds
  -- option.  HEVO trials use this to keep their narrow puzzle corridors
  -- readable even when the player selected VERY HIGH density globally.
  local encounterMax = tonumber(encDef and encDef.kaMaxVisible)
  if encounterMax and encounterMax >= 0 then
    maxSpawns = math.min(maxSpawns, math.floor(encounterMax))
  end
  local target = self.targetSpawnCount or maxSpawns
  target = math.min(target, maxSpawns)
...
function SpawnLogic:applySpawnAmount(value, source)
  ...
  local mapSpan = math.max(ow.map.widthCells or 0, ow.map.heightCells or 0)
  local encDef = self:_encDef(self.activeMapId, gameOf(self.mod))
  local maxVisible = Config.maxVisible(self.mod)
  local encounterMax = tonumber(encDef and encDef.kaMaxVisible)
  if encounterMax and encounterMax >= 0 then
    maxVisible = math.min(maxVisible, math.floor(encounterMax))
