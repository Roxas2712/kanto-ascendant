...
function AnimatedSprites.resolveSpeciesId(speciesKey, game, mod)
  ...
  local identity = mod and mod.speciesRuntimeIdentity
  if identity and type(identity.sourceDex) == "function" then
    local source = identity.sourceDex(speciesKey,
      game and game.data or nil)
    if source then return math.floor(source) end
  end
  ...
  if mon and (mon.sourceDex ~= nil or mon.dex ~= nil) then
    -- Mods may keep a save-stable private catalogue slot in `dex` while
    -- exposing the canonical National-Dex art identity as `sourceDex`.
    -- Wilds sheets and voxel billboards are National-Dex keyed, so the
    -- explicit source identity must win. Numeric inputs remain literal for
    -- compatibility with callers that already resolved a National Dex id.
    local dex = tonumber(mon.sourceDex or mon.dex)
...
AnimatedSprites.supportsSourceDex = true
...
function AnimatedSprites:getBillboardCard(speciesId, animName, direction, frameIndex, variant)
  ...
  local okBuild, imgOrErr = pcall(function()
    ...
    -- LÖVE Images are userdata on the supported 11.x runtime and cannot
    -- carry Lua fields.  The card cache above is the authoritative identity;
    -- writing diagnostic tags onto the userdata made every actual voxel-card
    -- conversion fail before it could reach the renderer.
    return img
  end)
