...
local function fsExists(path)
  return type(V.exists) == "function" and V.exists(path) == true
end
...
function SpriteProviders:_goldPackChoice(_game)
  -- Engine 0.1.86 deliberately keeps every mod's option bucket private.
  -- Only an explicit public export may disclose Gold's selected pack.
  local mod = self.mod
  local ok, hit = false, nil
  if mod and type(mod.find) == "function" then
    ok, hit = pcall(function() return mod.find(GOLD_MOD_ID) end)
  end
  local ex = ok and type(hit) == "table" and hit.exports or nil
  if type(ex) == "table" then
    local pack = ex.spritePack
    if type(ex.getSpritePack) == "function" then
      local called, value = pcall(ex.getSpritePack)
      if called then pack = value end
    end
    if pack == "gold" or pack == "silver" then return pack end
  end
  return "gold"
end
