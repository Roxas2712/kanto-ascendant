...
-- Sheets live under assets/bundled_runtime/followsprites_runtime/ (build-time).
--
-- Path types (do not mix):
--   relativePath  — mod-root relative, e.g. assets/bundled_runtime/.../001-normal.png
--   loadPath      — love.filesystem path via mod.assets:path(relativePath)
--                   e.g. mods/overworld_wild_spawns/assets/bundled_runtime/.../001-normal.png
--
...
RuntimeSheets.DIR_REL = "assets/bundled_runtime/followsprites_runtime"
...
function RuntimeSheets:probeRegistration(speciesId, variant)
  ...
  local getInfoRel = rel and (self:_assetPresent(rel) and "FOUND" or "MISSING")
    or nil
  return {
    speciesId = n,
