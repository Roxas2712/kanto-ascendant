...
env=dict(os.environ,TRAINER_REMATCH_MOD_DIR=str(root),TRACE_FIX_ROOT=str(root),GEN1RECOMP_DIR=str(a.engine.resolve()),GEN1RECOMP_TEST_DIR=str(a.engine_tests.resolve()),TRACE_TEST_ROOT=str(here))
rows=[]
for test in sorted((here/"tests").glob("*.lua")):
    if test.name == "headless_modkit_asset_sink.lua": continue
