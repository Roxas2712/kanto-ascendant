...
return function(mod,opts)
  ...
  local reasons={
    klutz={'Klutz disables the held effect; the item is still carried.','Tollpatsch deaktiviert die Wirkung; das Item bleibt getragen.'},
    embargo={'Embargo temporarily disables the held effect; the item remains carried.',
      'Itemsperre deaktiviert vorübergehend die Tragewirkung; das Item bleibt getragen.'},
    knocked_off={'Knocked off: inactive.','Abgeschlagen: inaktiv.'},
    no_item={'No held item.','Kein getragenes Item.'},
    item_preview={'Preview; not held yet.','Vorschau; noch nicht getragen.'},
    field_evolution_item={'Happiny: level up by day holding this stone. No battle bonus.',
      'Wonneira: Levelaufstieg am Tag mit diesem Stein. Kein Kampfbonus.'},
    ...
    no_held_effect={'No effect while held.','Kein Effekt beim Tragen.'},
    ...
    setting_save_failed={'Saving failed. The setting was restored.','Speichern fehlgeschlagen. Die Einstellung wurde zurückgesetzt.'},
  ...
  local function item(id,gen,game)
    ...
    local evolutionItems=mod.exports and mod.exports.wave1Happiny67
    ...
    local contact=mod.exports and mod.exports.pokemonContactAbilities67
    local lifeOrb=mod.exports and mod.exports.pokemonLifeOrb67
    local gems=mod.exports and mod.exports.pokemonGems67
    local heldStats=mod.exports and mod.exports.pokemonHeldStats67
    local grounding=mod.exports and mod.exports.pokemonGrounding67
    local trapping=mod.exports and mod.exports.backendPartialTrapping67
    local honey=mod.exports and mod.exports.pokemonHoneyGather67
    local pickup=mod.exports and mod.exports.pokemonPickup67
    local typeItems=mod.exports and mod.exports.pokemonTypeItems67
    local transformations=mod.exports and mod.exports.pokemonLateTransformations67
    local goggles=mod.exports and mod.exports.pokemonSafetyGoggles67
    local metadata=facts.item(id,gen) or facts.item(id,2)
      or evolutionItems and evolutionItems.item and evolutionItems.item(id)
      or sneasler and sneasler.item and sneasler.item(id)
      or cord and cord.item and cord.item(id)
      or choice and choice.itemMetadata and choice.itemMetadata(id)
      or contact and contact.itemMetadata and contact.itemMetadata(id)
      or lifeOrb and lifeOrb.itemMetadata and lifeOrb.itemMetadata(id)
      or gems and gems.itemMetadata and gems.itemMetadata(id)
      or heldStats and heldStats.itemMetadata and heldStats.itemMetadata(id)
      or grounding and grounding.itemMetadata and grounding.itemMetadata(id)
      or trapping and trapping.itemMetadata and trapping.itemMetadata(id)
      or honey and honey.itemMetadata(id)
      or pickup and pickup.itemMetadata(id)
      or typeItems and typeItems.itemMetadata(id)
      or transformations and transformations.itemMetadata(id)
      or goggles and goggles.itemMetadata(id)
    ...
    return {id=id,names=names,generation=metadata.generation,holdable=holdable,
      kascTypeItem=metadata.kascTypeItem,kascTypeItemKind=metadata.kascTypeItemKind,
      fieldEvolutionOnly=metadata.fieldEvolutionOnly==true,
      noHeldEffect=metadata.noHeldEffect==true or pickup and pickup.noHeldEffect(id)or false,
  ...
  local core=opts.makeCore({modId=mod.id,owned=localRows,item=item,generation=epoch,
    knockedItem=function(game,mon)
      local transfer=mod.exports and mod.exports.pokemonItemTransfer67
      return transfer and transfer.displayItem(game,mon)
    end,
    ...
    itemSuppressed=function(game,mon,ability,id,gen)
      local owner=mod.exports and mod.exports.pokemonHeldEffect67
      return owner and owner.itemSuppressed(game,mon,ability,id,gen)or false
    end,
    itemSuppressionView=function(game,mon)
      local owner=mod.exports and mod.exports.pokemonItemAccess67
      return owner and owner.uiSuppression(game,mon)
    end})
  local U={schema=core.schema,version=1,owner=core.OWNER,CARD_ID='KASC-67-EQUIPMENT-VIEW',
    ...
    heldId=core.held}
  function U.battleHeldId(mon,b,w)
    local owner=mod.exports and mod.exports.pokemonHeldEffect67
    if owner then return owner.held(mon,b,w)end
    return core.held(mon)
  end
  ...
  function U.help(kind,value,gen)
    value=value or {id=false}
    -- Do not disclose a reserved ability or egg contents through help.
    local description=value.id and opts.help and opts.help.describe(kind,value.id,gen,value)
    local reason=value.reasonCode and U.reason(value.reasonCode)
    local status=reason or {en='Active.',de='Aktiv.'}
    if value.reasonCode=='embargo'and type(value.suppression)=='table'
        and type(value.suppression.turns)=='number'then
      status={en=status.en..' '..value.suppression.turns..' turn(s) remaining.',
        de=status.de..' Noch '..value.suppression.turns..' Runde(n).'}
    end
    if description and value.reasonCode=='inactive_generation' then
      status={en='Dormant: rule era.',de='Inaktiv: Regelprofil.'}
    elseif description and value.reasonCode=='effect_not_implemented' then
      status={en='Not implemented.',de='Nicht implementiert.'}
    end
    if value.reasonCode=='field_evolution_item' and description then
      status={en='No battle bonus.',de='Kein Kampfbonus.'}
    end
    if description then return {schema='kasc.equipment-help/v1',
      en=description.en..' '..status.en,de=description.de..' '..status.de} end
    return {schema='kasc.equipment-help/v1',
      en=(value.id and 'Effect description pending. ' or '')..status.en,
      de=(value.id and 'Wirkungsbeschreibung fehlt noch. ' or '')..status.de}
  end
  local function helpText(kind,value,gen)
    local result=U.help(kind,value,gen);return tr(result.en,result.de)
  end
  ...
  local function menu(game,title,rows,onChoose)
    local list=(mod.ui.KantoListMenu or mod.ui.ListMenu).new(game,title,rows,
      {pageJump=true,ascendantFocusHelp=function(row)return row.help end,onChoose=onChoose})
  ...
  function U.openNative(game,monOrHandle)
    ...
    -- Existing identities can be inspected during battle without starting
    -- an unsafe save transaction. Commands stay guarded by core.snapshot.
    if not handle or rules.safeBoundary(game)then
      local ready,reason=U.initialize(game)
      if not ready then notice(game,reason);return nil,reason end
      handle=type(monOrHandle)=='table'and monOrHandle._kascEquipmentHandle or monOrHandle
    end
    ...
    local rows={{label=tr('ITEM: ','ITEM: ')..name(view.item),value='item',
      help=helpText('item',view.item,view.generation)},
      {label=tr('ABILITY: ','FÄHIGKEIT: ')..name(view.ability),value='ability',
       help=helpText('ability',view.ability,view.generation)}}
    ...
    local currentMon
    for _,row in ipairs(localRows(game))do
      if row.mon._kascEquipmentHandle==handle then currentMon=row.mon end
    end
    if (view.commands.give or view.commands.take or view.commands.swap)
        and evolutionItems and evolutionItems.canBuy and evolutionItems.canBuy(game,currentMon) then
      rows[#rows+1]={label=tr('OVAL STONE SHOP','OVALSTEIN KAUFEN'),value='buy_oval',
        help=tr('PRICE: 2000','PREIS: 2000')}
    end
    local familyItems=mod.exports and mod.exports.wave1EvolutionItems67
    if familyItems and familyItems.canBuy(game,currentMon) then
      rows[#rows+1]={label=tr('EVOLUTION ITEMS','ENTWICKLUNGSITEMS'),value='buy_evolution',
        help=tr('Buy evolution items. Read each item\'s usage instructions.',
          'Entwicklungsitems kaufen. Beachte den Anwendungshinweis des jeweiligen Items.')}
    end
    local inkay=mod.exports and mod.exports.wave1Inkay67
    local inversion=inkay and inkay.choice(game,currentMon)
    if inversion then rows[#rows+1]=inversion end
    ...
    return menu(game,tr('EQUIPMENT','AUSRÜSTUNG'),rows,function(selected,list)
      ...
      if action=='inkay_invert' and inversion then
        local ok,why=inkay.setInverted(game,currentMon,not inversion.enabled,inversion.enabled)
        list:close()
        if not ok then notice(game,why);return end
        U.openNative(game,handle);return
      end
      if action=='buy_evolution' then
        list:close()
        familyItems.openShop(game,currentMon,function() U.openNative(game,handle) end)
        return
      end
      if action=='buy_oval' then
        list:close()
        evolutionItems.openShop(game,currentMon,function() U.openNative(game,handle) end)
        return
      end
      ...
      for _,value in ipairs(inventory.items)do
        local descriptor={id=value.id,reasonCode='item_preview'}
        choices[#choices+1]={label=name(value),right=tostring(value.quantity),value=value.id,
          help=helpText('item',descriptor,view.generation)}
      end
