...
-- exact .zip/.modpkg path and verifies the manager sees 5.2.1.
...
return function()
  ...
  assert(ok and id == "trainer_rematch",
    "5.2.1 .modpkg update failed: " .. tostring(id))
  local currentVersion, row = installedVersion()
  assert(currentVersion == "5.2.1",
    "launcher did not expose installed 5.2.1: " .. tostring(currentVersion))
  ...
  assert(manifest:find('"version"%s*:%s*"5%.2%.1"'),
