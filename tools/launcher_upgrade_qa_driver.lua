...
-- exact .zip/.modpkg path and verifies the manager sees 5.4.2.
...
return function()
  ...
  assert(oldVersion == "5.4.1",
    "launcher did not expose installed 5.4.1: " .. tostring(oldVersion))

  ok, id = LauncherMods.installZip(currentPackage, {
    replace = true, expectId = "trainer_rematch",
  })
  assert(ok and id == "trainer_rematch",
    "5.4.2 .modpkg update failed: " .. tostring(id))
  local currentVersion, manifest = installedVersion()
  assert(currentVersion == "5.4.2",
    "launcher did not expose installed 5.4.2: " .. tostring(currentVersion))
  assert(manifest:find('"version"%s*:%s*"5%.4%.2"'),
    "installed tree does not contain the current manifest")

  print(("LAUNCHER PACKAGE QA PASS: 5.4.1 -> %s (%s)")
