...
-- four player-assigned Bag items. Registration lives on SELECT inside the
-- Bag; the original item-reorder action remains available from that screen.
--
...
return function(mod)
  ...
  local function registrationRows(game, selectedId, list, originalSelect)
    local rows = {}
  ...
  local function openRegistration(game, item, list, originalSelect)
    if not item or type(item.value) ~= "string" then return end
    local selectedId = item.value
    local rows = registrationRows(game, selectedId, list, originalSelect)
    local picker
    picker = mod.ui.ListMenu.new(game, tr("QUICK SELECT", "SCHNELLWAHL"),
      rows, {
        onChoose = function(choice, menu)
          if choice.value == "move" then
            menu:close()
            originalSelect(item, list)
            return
          end
