...
return function(opts)
  ...
  local function descriptor(id,gen,game)
    ...
    if item.fieldEvolutionOnly then
      result.active=false -- No battle effect, including in older rule profiles.
      result.reasonCode='field_evolution_item'
    end
    if item.noHeldEffect then
      result.active=false
      result.reasonCode='no_held_effect'
    end
    return result
  end
  function M.snapshot(game,handle)
    ...
    if snapshot.item.active and opts.itemSuppressed
        and opts.itemSuppressed(game,row.mon,ability,item,gen)then
      snapshot.item.active=false;snapshot.item.reasonCode='klutz'
    end
    local suppression=not snapshot.isEgg and snapshot.item.active and opts.itemSuppressionView
      and opts.itemSuppressionView(game,row.mon)
    if type(suppression)=='table'and suppression.reasonCode=='embargo'then
      snapshot.item.active=false;snapshot.item.reasonCode='embargo'
      snapshot.item.suppression=copy(suppression)
    end
    ...
    local knocked=not snapshot.isEgg and opts.knockedItem and opts.knockedItem(game,row.mon)
    if knocked then
      snapshot.item=descriptor(knocked,gen,game)
      snapshot.item.active=false;snapshot.item.reasonCode='knocked_off'
      snapshot.commands.give=false;snapshot.commands.take=false;snapshot.commands.swap=false
      snapshot.reasonCode='knocked_off'
    end
    return snapshot
  end
