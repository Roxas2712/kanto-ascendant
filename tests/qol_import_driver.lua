...
-- kanto_ascendant folder inside that identity.
...
return function()
  ...
  check("launcher importer accepts the archive",
    installed == true and result == "kanto_ascendant", result)

  local manifest = love.filesystem.read("mods/kanto_ascendant/manifest.json")
  ...
  local hasFireRed = love.filesystem.getInfo(
    "mods/kanto_ascendant/modern_storage_ui.lua", "file") ~= nil
  local hasHub = love.filesystem.getInfo(
    "mods/kanto_ascendant/ascendant_features.lua", "file") ~= nil
  check(expectFireRed
